...
export default function ReadingTests({ tests }: { tests: Test[] }) {
    return (
        <div className="space-y-4 container mx-auto grid grid-cols-3 grid-rows-4 gap-4">
            {tests.map((test) => (
                <div key={test.test_slug} className="p-4 border rounded-lg h-full flex flex-col justify-between">
                    <div>
                    ...
                    <Link 
                        ...
                        className="bg-blue-500 text-white px-4 py-2 rounded-md max-w-fit hover:bg-blue-600 transition-colors"
                    >
                        Start test
                    </Link>
                </div>
